...
int ReceivePoint(igtl::Socket * socket, igtl::MessageHeader * header)
{
  ...
  if (c & igtl::MessageHeader::UNPACK_BODY) // if CRC check is OK
    ...
    for (int i = 0; i < nElements; i ++)
	  ...
	  igtl::PointMessage::Pointer pointNewMsg;
	  pointNewMsg = igtl::PointMessage::New();
	  pointNewMsg->SetDeviceName("PointSender");
	   pointElement->SetPosition(-pos[0],-pos[1],-pos[2]);
	   pointNewMsg->AddPointElement(pointElement);
	  pointNewMsg->Pack();
		socket->Send(pointNewMsg->GetPackPointer(), pointNewMsg->GetPackSize());
